...
import com.zdy.springbootsecurity.Service.UploadVideoService;
...
public class UploadVideoServiceImpl implements UploadVideoService {
    ...
    @Override
    public ResponseResult saveVideoFile(MultipartFile file) throws IOException {
        ...
        // 创建目标文件对象
        File targetFile = new File(storageDirectoryPath, fileName);

        // 将上传文件保存到目标文件中
        file.transferTo(targetFile);

        // 调用视频服务的方法，将文件相关信息保存到数据库中
        userMapper.saveVideo(fileName, targetFile.getAbsolutePath(),user_id);
