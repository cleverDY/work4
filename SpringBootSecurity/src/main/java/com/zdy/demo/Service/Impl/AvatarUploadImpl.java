...
public class AvatarUploadImpl implements AvatarUploadService {
    ...
    @Override
    public ResponseResult uploadAvatar(MultipartFile file) throws ErrorException {
        ...
        if (!storageDirectory.exists()) {
            ...
            if (!wasSuccessful) {
                // 如果目录创建失败，可以返回一个错误响应
                throw new ErrorException(ErrorExceptionEnum.FILE_UPLOAD_FAILED);
            }
        }
        ...
        try {
            ...
            //保存用户头像url
            boolean status = modifyUserAvatarUrlById(user.getUser().getId(), file.getOriginalFilename());
            if(!status){
